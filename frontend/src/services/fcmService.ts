    ...
    if (Platform.OS === 'android' && Platform.Version >= 33) {
      const status = await PermissionsAndroid.request(
        PermissionsAndroid.PERMISSIONS.POST_NOTIFICATIONS,
        {
          ...
          message:
            'ShieldHire needs permission to send you notifications about bookings and alerts.',
...
export async function initFCM(
  ...
  _tokenRefreshUnsubscribe = messaging().onTokenRefresh(async newToken => {
...
async function displayForegroundNotification(
  ...
  try {
    ...
    await notifee.displayNotification({
      title: remoteMessage.notification?.title ?? 'ShieldHire',
      body:
