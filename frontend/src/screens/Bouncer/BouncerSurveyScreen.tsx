...
const STEPS = ['Info', 'Bio', 'Skills', 'Photos', 'Verify'];
...
export default function BouncerSurveyScreen({ navigation }: Props) {
    ...
    // Verification
    const [aadhaar, setAadhaar] = useState(user?.bouncerProfile?.aadhaarLast4 || '');
    const [isIdentityVerified, setIsIdentityVerified] = useState(user?.bouncerProfile?.identityVerified || false);
            ...
            try {
                ...
                if (response.data) {
                    ...
                    if (freshUser.bouncerProfile) {
                        ...
                        setAadhaar(bp.aadhaarLast4 || '');
                        setIsIdentityVerified(bp.identityVerified || false);
                    }
                }
            } catch (e: any) {
    ...
    useEffect(() => {
        if (user) {
            ...
            if (user.bouncerProfile) {
                ...
                if (!aadhaar && bp.aadhaarLast4) setAadhaar(bp.aadhaarLast4);
                if (!isIdentityVerified && bp.identityVerified) setIsIdentityVerified(bp.identityVerified);
            }
        }
    }, [user]);
    ...
    const handleNext = async () => {
        if (currentStep === 4 && !isIdentityVerified) {
            Alert.alert('Verification Required', 'Please complete identity verification to proceed.');
            return;
        }
        ...
        try {
            ...
            const response = await api.put('/user/profile', {
                ...
                bouncerProfile: {
                    ...
                    livenessVerifiedAt: isIdentityVerified ? new Date().toISOString() : null,
                }
            });
    ...
    const handleSubmit = async () => {
        if (!isIdentityVerified) {
            Alert.alert('Action Required', 'You must complete the verification step before submitting.');
            return;
        }
        await saveProfile();
    };
            ...
            case 4: // Verify
                return (
                    <View style={styles.stepContainer}>
                        ...
                        <Text style={styles.subText}>For identity verification and security compliance, please provide your Aadhaar details and complete a quick liveness scan.</Text>

                        <View style={styles.verifyCard}>
                            <View style={styles.verifyHeaderRow}>
                                <Ionicons name="shield-checkmark" size={24} color={isIdentityVerified ? "#4CAF50" : "#FFD700"} />
                            ...
                            <View style={styles.livenessSection}>
                                ...
                                {isIdentityVerified ? (
                                    ...
                                    >
                                        ...
                                        <Text style={styles.livenessBtnText}>Start Face Scan</Text>
                                    </TouchableOpacity>
                                )}
                            </View>
                        </View>
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <Modal visible={showLivenessCamera} animationType="slide">
                <LivenessCamera 
                    onSuccess={(result) => {
                        console.log('[Survey] Liveness verification succeeded with payload:', result);
